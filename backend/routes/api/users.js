  ...
  userModel.changePassword = (email, newPassword, handler) => {
    ...
    userColl.findOne(query, { projection: projection }, (err, user) => {
      ...
      if (bcrypt.compareSync(newPassword, user.password)) {
        return handler(
          new Error("Error: Debe usar una contraseña nueva"),
          null
        );
      }

      var oldPasswords = user.lastPasswords.filter((psw, i) => {
        return bcrypt.compareSync(newPassword, psw);
      });

      if (oldPasswords.lenght > 0) {
        return handler(
          new Error("Error: Debe usar una contraseña nueva 2"),
          null
        );
      }
      ...
      var update = {
        $set: {
          ...
          lastChangePassword: new Date().getTime(),
        },
      };
