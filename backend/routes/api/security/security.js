...
function initSecurity(db) {
      ...
      (email, pswd, next) => {
        if ((email || "na") === "na" || (pswd || "na") === "na") {
          ...
          return next(null, false, {
            Error: "El correo y la contraseña son necesarios",
          });
        }
        userModel.getByEmail(email, (err, user) => {
          if (err) {
            console.log(err);
            console.log(
              "Ocurrio un error al tratar de iniciar sesion ERR:EA-I" + email
            );
            return next(null, false, {
              Error: "El correo y la contraseña son necesarios",
            });
          ...
          if (!user.active) {
            console.log(
              "Ocurrio un error al tratar de iniciar sesion ERR:EA-U" + email
            );
            return next(null, false, {
              Error: "El correo y la contraseña son necesarios",
            });
          ...
          if (!userModel.comparePassword(pswd, user.password)) {
            console.log(
              "Ocurrio un error al tratar de iniciar sesion ERR:EC-I" + email
            );
            return next(null, false, {
              Error: "El correo y la contraseña son necesarios",
            });
  ...
  router.post("/signin", (req, res, next) => {
    ...
    if (email === "na" || pswd === "na") {
      return res
        .status(400)
        .json({ Error: "El correo y la contraseña son necesarios" });
    }
    if (
    ...
    if (
      ...
      return res.status(400).json({
        Error:
          "Necesarios 1 mayuscula, una minuscula, numero y un signo especial",
      });
    }
    ...
    userModel.addNew(email, pswd, (err, newUser) => {
      if (err) {
        return res
          .status(400)
          .json({ Error: "No se logro crear una nueva cuenta" });
      }
